...
ERRORS = {
    ...
    'ADCM_NOT_FOUND':
    ("adcm object doesn't exist", rfs.HTTP_404_NOT_FOUND, ERR),
    'BUNDLE_NOT_FOUND': ("bundle doesn't exist", rfs.HTTP_404_NOT_FOUND, ERR),
    'CLUSTER_NOT_FOUND':
    ("cluster doesn't exist", rfs.HTTP_404_NOT_FOUND, ERR),
    'SERVICE_NOT_FOUND':
    ("service doesn't exist", rfs.HTTP_404_NOT_FOUND, ERR),
    'BIND_NOT_FOUND': ("bind doesn't exist", rfs.HTTP_404_NOT_FOUND, ERR),
    'PROVIDER_NOT_FOUND': ("provider doesn't exist", rfs.HTTP_404_NOT_FOUND,
                           ERR),
    'HOST_NOT_FOUND': ("host doesn't exist", rfs.HTTP_404_NOT_FOUND, ERR),
    'HOST_TYPE_NOT_FOUND': ("host type doesn't exist", rfs.HTTP_404_NOT_FOUND,
                            ERR),
    'PROTOTYPE_NOT_FOUND': ("prototype doesn't exist", rfs.HTTP_404_NOT_FOUND,
                            ERR),
    'HOSTSERVICE_NOT_FOUND': ("map host <-> component doesn't exist",
                              rfs.HTTP_404_NOT_FOUND, ERR),
    'COMPONENT_NOT_FOUND': ("component doesn't exist", rfs.HTTP_404_NOT_FOUND,
                            ERR),
    'ACTION_NOT_FOUND': ("action for service doesn't exist",
                         rfs.HTTP_404_NOT_FOUND, ERR),
    'CLUSTER_SERVICE_NOT_FOUND':
    ("service is not installed in specified cluster", rfs.HTTP_404_NOT_FOUND,
     ERR),
    ...
    'USER_NOT_FOUND': ("user profile is not found", rfs.HTTP_404_NOT_FOUND,
                       ERR),
    'GROUP_NOT_FOUND': ("group is not found", rfs.HTTP_404_NOT_FOUND, ERR),
    'ROLE_NOT_FOUND': ("role is not found", rfs.HTTP_404_NOT_FOUND, ERR),
    'PERMISSION_NOT_FOUND': ("permission is not found", rfs.HTTP_404_NOT_FOUND,
                             ERR),
    'MODULE_NOT_FOUND': ("module doesn't exist", rfs.HTTP_404_NOT_FOUND, ERR),
    'FUNCTION_NOT_FOUND': ("function doesn't exist", rfs.HTTP_404_NOT_FOUND,
                           ERR),
    'TASK_GENERATOR_ERROR': ("task generator error", rfs.HTTP_409_CONFLICT,
                             ERR),
    'OBJ_TYPE_ERROR': ("wrong object type", rfs.HTTP_409_CONFLICT, ERR),
    'SERVICE_CONFLICT': ("service already exists in specified cluster",
                         rfs.HTTP_409_CONFLICT, ERR),
    'CLUSTER_CONFLICT': ("duplicate cluster", rfs.HTTP_409_CONFLICT, ERR),
    'PROVIDER_CONFLICT': ("duplicate host provider", rfs.HTTP_409_CONFLICT,
                          ERR),
    ...
    'FOREIGN_HOST': ("host is not belong to the cluster",
                     rfs.HTTP_409_CONFLICT, ERR),
    'COMPONENT_CONFLICT': ("duplicate component on host in cluster",
                           rfs.HTTP_409_CONFLICT, ERR),
    'COMPONENT_CONSTRAINT_ERROR': ("component constraint error",
                                   rfs.HTTP_409_CONFLICT, ERR),
    ...
    'INVALID_OBJECT_DEFINITION': ("invalid object definition",
                                  rfs.HTTP_409_CONFLICT, ERR),
    'INVALID_CONFIG_DEFINITION': ("invalid config definition",
                                  rfs.HTTP_409_CONFLICT, ERR),
    'INVALID_COMPONENT_DEFINITION': ("invalid component definition",
                                     rfs.HTTP_409_CONFLICT, ERR),
    'INVALID_ACTION_DEFINITION': ("invalid action definition",
                                  rfs.HTTP_409_CONFLICT, ERR),
    'INVALID_UPGRADE_DEFINITION': ("invalid upgrade definition",
                                   rfs.HTTP_409_CONFLICT, ERR),
    'INVALID_VERSION_DEFINITION': ("invalid version definition",
                                   rfs.HTTP_409_CONFLICT, ERR),
    'INVALID_OBJECT_UPDATE': ("invalid update of object definition",
                              rfs.HTTP_409_CONFLICT, ERR),
    'INVALID_CONFIG_UPDATE': ("invalid update of config definition",
                              rfs.HTTP_409_CONFLICT, ERR),
    'BUNDLE_ERROR': ("bundle error", rfs.HTTP_409_CONFLICT, ERR),
    'BUNDLE_VERSION_ERROR': ("bundle version error", rfs.HTTP_409_CONFLICT,
                             ERR),
    ...
    'CONFIG_NOT_FOUND': ("config param doesn't exist", rfs.HTTP_404_NOT_FOUND,
                         ERR),
    'SERVICE_CONFIG_ERROR': ("service config parsing error",
                             rfs.HTTP_409_CONFLICT, ERR),
    ...
    'NOT_ALLOWED_TERMINATION': ("not allowed termination the task",
                                rfs.HTTP_409_CONFLICT, ERR),
    'WRONG_SELECTOR': ("selector error", rfs.HTTP_409_CONFLICT, ERR),
    'WRONG_JOB_TYPE': ("unknown job type", rfs.HTTP_409_CONFLICT, ERR),
    'WRONG_ACTION_CONTEXT': ("unknown action context", rfs.HTTP_409_CONFLICT,
                             ERR),
    'WRONG_ACTION_TYPE': ("config action type error", rfs.HTTP_409_CONFLICT,
                          ERR),
    'WRONG_ACTION_HC': ("action hostcomponentmap error", rfs.HTTP_409_CONFLICT,
                        ERR),
    'WRONG_CLUSTER_ID_TYPE': ("cluster id must be integer",
                              rfs.HTTP_400_BAD_REQUEST, ERR),
    ...
    'CONFIG_KEY_ERROR': ("error in json config", rfs.HTTP_400_BAD_REQUEST,
                         ERR),
    'CONFIG_VALUE_ERROR': ("error in json config", rfs.HTTP_400_BAD_REQUEST,
                           ERR),
    'ATTRIBUTE_ERROR': ("error in attribute config", rfs.HTTP_400_BAD_REQUEST,
                        ERR),
    'TOO_LONG': ("response is too long", rfs.HTTP_400_BAD_REQUEST, WARN),
    'NOT_IMPLEMENTED': ("not implemented yet", rfs.HTTP_501_NOT_IMPLEMENTED,
                        ERR),
