...
from cm.models import (Prototype, Component, Host, HostComponent,
                       ServiceComponent, PrototypeImport, ClusterBind,
                       ClusterObject, Upgrade)
...
def check_license(bundle):
    if bundle.license == 'unaccepted':
        ...
        err('LICENSE_ERROR',
            msg.format(bundle.name, bundle.version, bundle.edition))
...
def switch_service(co, new_proto):
    log.info('upgrade switch from %s to %s', proto_ref(co.prototype),
             proto_ref(new_proto))
...
def switch_components(cluster, co, new_co_proto):
    for sc in ServiceComponent.objects.filter(cluster=cluster, service=co):
        try:
            comp = Component.objects.get(prototype=new_co_proto,
    ...
    for comp in Component.objects.filter(prototype=new_co_proto):
        try:
            ServiceComponent.objects.get(cluster=cluster,
                                         service=co,
                                         component=comp)
...
def switch_hc(obj, upgrade):
    ...
    def find_component(component, proto):
        try:
            return Component.objects.get(prototype=proto,
