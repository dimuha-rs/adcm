...
STAGE = (StagePrototype, StageComponent, StageAction, StagePrototypeConfig,
...
def update_bundle(bundle):
    try:
        check_stage()
        process_bundle(os.path.join(config.BUNDLE_DIR, bundle.hash),
                       bundle.hash)
...
def untar(bundle_hash, bundle):
    ...
    if os.path.isdir(path):
        err('BUNDLE_ERROR',
            'bundle directory "{}" already exists'.format(path))
...
def process_bundle(path, bundle_hash):
    obj_list = {}
    for conf_path, conf_file, conf_type in cm.stack.get_config_files(
            path, bundle_hash):
        conf = cm.stack.read_definition(conf_file, conf_type)
        if conf:
            cm.stack.save_definition(conf_path, conf_file, conf, obj_list,
                                     bundle_hash)
...
def re_check_actions():
    for act in StageAction.objects.all():
        ...
        ref = 'in hc_acl of action "{}" of {}'.format(act.name,
                                                      proto_ref(act.prototype))
        for item in hc:
            sp = StagePrototype.objects.filter(type='service',
                                               name=item['service'])
            if not sp:
                msg = 'Unknown service "{}" {}'
                err('INVALID_ACTION_DEFINITION',
                    msg.format(item['service'], ref))
            if not StageComponent.objects.filter(prototype=sp[0],
                                                 name=item['component']):
                msg = 'Unknown component "{}" of service "{}" {}'
                err('INVALID_ACTION_DEFINITION',
                    msg.format(item['component'], sp[0].name, ref))
...
def re_check_components():
    for comp in StageComponent.objects.all():
        if not comp.requires:
            continue
        ref = 'in requires of component "{}" of {}'.format(
            comp.name, proto_ref(comp.prototype))
        req_list = comp.requires
        for i, item in enumerate(req_list):
            if 'service' in item:
                try:
                    service = StagePrototype.objects.get(name=item['service'],
                                                         type='service')
            ...
            try:
                req_comp = StageComponent.objects.get(name=item['component'],
                                                      prototype=service)
            except StageComponent.DoesNotExist:
                msg = 'Unknown component "{}" {}'
                err('COMPONENT_CONSTRAINT_ERROR',
                    msg.format(item['component'], ref))
            if comp == req_comp:
...
def re_check_config():
    for c in StagePrototypeConfig.objects.filter(type='variant'):
        ...
        if len(keys) > 1:
            subname = keys[1]
        try:
            s = StagePrototypeConfig.objects.get(prototype=c.prototype,
                                                 name=name,
                                                 subname=subname)
...
def copy_stage_prototype(stage_prototypes, bundle):
    ...
    for sp in stage_prototypes:
        p = copy_obj(
            sp, Prototype,
            ('type', 'path', 'name', 'version', 'required', 'shared',
             'monitoring', 'display_name', 'description', 'adcm_min_version'))
        p.bundle = bundle
        prototypes.append(p)
...
def copy_stage_actons(stage_actions, prototype):
    actions = prepare_bulk(
        stage_actions, Action, prototype,
        ('name', 'type', 'script', 'script_type', 'state_on_success',
         ...
         'ui_options', 'allow_to_terminate', 'partial_execution'))
...
def copy_stage_sub_actons(bundle):
    ...
    for ssubaction in StageSubAction.objects.all():
        ...
        sub = copy_obj(ssubaction, SubAction,
                       ('name', 'display_name', 'script', 'script_type',
                        'state_on_fail', 'params'))
        sub.action = action
...
def copy_stage_component(stage_components, prototype):
    components = prepare_bulk(stage_components, Component, prototype,
                              ('name', 'display_name', 'description', 'params',
                               'monitoring', 'requires', 'constraint'))
    Component.objects.bulk_create(components)


def copy_stage_import(stage_imports, prototype):
    imports = prepare_bulk(stage_imports, PrototypeImport, prototype,
                           ('name', 'min_version', 'max_version', 'min_strict',
                            'max_strict', 'default', 'required', 'multibind'))
...
def copy_stage_config(stage_config, prototype):
    ...
    for sc in stage_config:
        ...
        if sc.action:
            c.action = Action.objects.get(prototype=prototype,
                                          name=sc.action.name)
...
def check_license(bundle):
    b = Bundle.objects.filter(license_hash=bundle.license_hash,
                              license='accepted')
    if not b:
        return False
    return True
...
def update_bundle_from_stage(bundle):  # pylint: disable=too-many-locals,too-many-branches,too-many-statements
    for sp in StagePrototype.objects.all():
        try:
            p = Prototype.objects.get(bundle=bundle,
                                      type=sp.type,
                                      name=sp.name,
                                      version=sp.version)
            p.path = sp.path
            ...
            p = copy_obj(sp, Prototype,
                         ('type', 'path', 'name', 'version', 'required',
                          'shared', 'monitoring', 'display_name',
                          'description', 'adcm_min_version'))
            p.bundle = bundle
        p.save()
        for scomp in StageComponent.objects.filter(prototype=sp):
            try:
                ...
                update_obj(comp, scomp,
                           ('display_name', 'description', 'params',
                            'monitoring', 'requires', 'constraint'))
            except Component.DoesNotExist:
                comp = copy_obj(scomp, Component,
                                ('name', 'display_name', 'description',
                                 'params', 'requires', 'constraint'))
                comp.prototype = p
            comp.save()
            ...
            try:
                ...
                update_obj(
                    ...
                    ('type', 'script', 'script_type', 'state_on_success',
                     'state_on_fail', 'state_available', 'params', 'log_files',
                     'hostcomponentmap', 'button', 'display_name',
                     'description', 'ui_options', 'allow_to_terminate',
                     'partial_execution'))
            except Action.DoesNotExist:
                action = copy_obj(
                    saction, Action,
                    ('name', 'type', 'script', 'script_type',
                     'state_on_success', 'state_on_fail', 'state_available',
                     'params', 'log_files', 'hostcomponentmap', 'button',
                     'display_name', 'description', 'ui_options',
        ...
        for sc in StagePrototypeConfig.objects.filter(prototype=sp):
            flist = ('default', 'type', 'description', 'display_name',
                     'limits', 'required', 'ui_options')
            act = None
            if sc.action:
                act = Action.objects.get(prototype=p, name=sc.action.name)
            try:
                pconfig = PrototypeConfig.objects.get(prototype=p,
                                                      action=act,
                                                      name=sc.name,
                                                      subname=sc.subname)
                update_obj(pconfig, sc, flist)
            except PrototypeConfig.DoesNotExist:
                pconfig = copy_obj(sc, PrototypeConfig,
                                   ('name', 'subname') + flist)
                pconfig.action = act
                pconfig.prototype = p
            pconfig.save()
    ...
    for su in StageUpgrade.objects.all():
        upg = copy_obj(su, Upgrade,
                       ('name', 'description', 'min_version', 'max_version',
                        'min_strict', 'max_strict', 'state_available',
                        'state_on_success', 'from_edition'))
        upg.bundle = bundle
        upg.save()
