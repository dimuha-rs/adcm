...
class TestJob(TestCase):
    ...
    def test_set_job_status(self):
        ...
        job = models.JobLog.objects.create(action_id=action.id,
    ...
    def test_set_task_status(self):
        event = Mock()
        task = models.TaskLog.objects.create(action_id=1,
                                             object_id=1,
                                             start_date=timezone.now(),
                                             finish_date=timezone.now())
        ...
        event.set_task_status.assert_called_once_with(task.id,
                                                      config.Job.RUNNING)
    ...
    def test_get_task_obj(self):
        ...
        cluster_object = models.ClusterObject.objects.create(
            prototype=prototype, cluster=cluster)
    ...
    def test_get_state(self):
        ...
        cluster = models.Cluster.objects.create(prototype=prototype)
        action = models.Action.objects.create(prototype=prototype,
                                              state_on_success='create',
                                              state_on_fail='installed')

        job = models.JobLog(action_id=action.id,
                            selector={'cluster': cluster.id},
                            start_date=timezone.now(),
                            finish_date=timezone.now())

        data = [(config.Job.SUCCESS, False, 'create'),
        ...
        for status, create_sub_action, test_state in data:
            with self.subTest(status=status,
                              create_sub_action=create_sub_action,
    ...
    def test_set_action_state(self, mock_push_obj):
        ...
        adcm = models.ADCM.objects.create(prototype=prototype)
        action = models.Action.objects.create(prototype=prototype)
        task = models.TaskLog.objects.create(action_id=action.id,
                                             object_id=1,
                                             start_date=timezone.now(),
                                             finish_date=timezone.now())
    ...
    def test_unlock_obj(self, mock_set_object_state):
        ...
        data = [
            (Mock(stack=['running']),
             mock_set_object_state.assert_called_once),
        ...
        for obj in data:
            with self.subTest(obj=obj):
                ...
                if isinstance(obj, models.HostProvider):
                    mock_unlock_obj.assert_has_calls([call(obj, event)])
    ...
    def test_restore_hc(self, mock_save_hc):
        ...
        mock_save_hc.assert_called_once_with(
            cluster, [(cluster_object, host, service_component)])
    ...
    def test_get_action_context(self):
        ...
        data = [
            ({
                'cluster': cluster.id
            }, 'service', cluster_object, cluster),
            ({
                'host': host.id
            }, 'host', host, cluster),
            ({
        ...
        for selector, prototype_type, test_obj, test_cluster in data:
            with self.subTest(selector=selector,
                ...
                obj, _cluster, _provider = job_module.get_action_context(
                    action, selector)
    ...
    def test_prepare_job(self, mock_prepare_job_inventory,
                         mock_prepare_job_config, mock_prepare_ansible_config):
        bundle = models.Bundle.objects.create()
        prototype = models.Prototype.objects.create(bundle=bundle)
        cluster = models.Cluster.objects.create(prototype=prototype)
        action = models.Action.objects.create(prototype=prototype)
        ...
        mock_prepare_job_inventory.assert_called_once_with({'cluster': 1},
                                                           job.id, {}, None)
        mock_prepare_job_config.assert_called_once_with(
            action, None, {'cluster': 1}, job.id, cluster, '')
    ...
    def test_get_bundle_root(self):
        ...
        data = [('adcm', os.path.join(config.BASE_DIR, 'conf')),
                ('', config.BUNDLE_DIR)]
    ...
    def test_cook_script(self, mock_get_bundle_root):
        bundle = models.Bundle.objects.create(
            hash='6525d392dc9d1fb3273fb4244e393672579d75f3')
        prototype = models.Prototype.objects.create(bundle=bundle)
        action = models.Action.objects.create(prototype=prototype)
        sub_action = models.SubAction.objects.create(
            action=action, script='ansible/sleep.yaml')
        mock_get_bundle_root.return_value = config.BUNDLE_DIR

        data = [
            (sub_action, 'main.yaml',
             os.path.join(config.BUNDLE_DIR, action.prototype.bundle.hash,
                          'ansible/sleep.yaml')),
            (None, 'main.yaml',
             os.path.join(config.BUNDLE_DIR, action.prototype.bundle.hash,
                          'main.yaml')),
    ...
    @patch("json.dump")
    @patch("builtins.open")
    def test_prepare_job_config(self, mock_open, mock_dump,
                                mock_get_adcm_config, mock_prepare_context,
                                mock_get_bundle_root, mock_cook_script):
        bundle = models.Bundle.objects.create()
        prototype = models.Prototype.objects.create(bundle=bundle)
        cluster = models.Cluster.objects.create(prototype=prototype)
        cluster_object = models.ClusterObject.objects.create(
        ...
        for prototype_type, obj in data:
            with self.subTest(provider_type=prototype_type, obj=obj):
                ...
                job_module.prepare_job_config(action, sub_action, selector,
                                              job.id, obj, conf)
                ...
                job_config = {
                    ...
                    'env': {
                        'run_dir':
                        mock_dump.call_args[0][0]['env']['run_dir'],
                        'log_dir':
                        mock_dump.call_args[0][0]['env']['log_dir'],
                        'tmp_dir':
                        os.path.join(config.RUN_DIR, f'{job.id}', 'tmp'),
                        'stack_dir':
                        mock_dump.call_args[0][0]['env']['stack_dir'],
                        'status_api_token':
                        mock_dump.call_args[0][0]['env']['status_api_token']
                    },
                    'job': {
                        'id': 1,
                        'action': '',
                        ...
                        'script': '',
                        'playbook':
                        mock_dump.call_args[0][0]['job']['playbook'],
                        'params': {
                ...
                mock_dump.assert_called_with(job_config,
                                             fd,
                                             indent=3,
                                             sort_keys=True)
    ...
    def test_re_prepare_job(self, mock_prepare_job, mock_get_new_hc,
        ...
        task = models.TaskLog.objects.create(action_id=action.id,
                                             ...
                                             selector=selector,
                                             config={"sleeptime": 1})
        job = models.JobLog.objects.create(task_id=task.id,
                                           action_id=action.id,
        ...
        mock_cook_delta.assert_called_once_with(cluster, new_hc,
                                                action.hostcomponentmap,
                                                old_hc)
        mock_prepare_job.assert_called_once_with(action, sub_action, selector,
                                                 job.id, cluster, task.config,
                                                 delta, None)
