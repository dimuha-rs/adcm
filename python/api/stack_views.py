...
class UploadBundle(GenericAPIPermView):
    ...
    authentication_classes = (CsrfOffSessionAuthentication,
                              TokenAuthentication)
    ...
    def post(self, request):
        ...
        try:
            serializer = self.serializer_class(data=request.data,
                                               context={'request': request})
            if serializer.is_valid():
                bundle = cm.bundle.load_bundle(
                    serializer.validated_data.get('bundle_file'))
                srl = api.stack_serial.BundleSerializer(
...
class ServiceDetail(DetailViewRO):
    ...
    def get_object(self):
        ...
        service.actions = Action.objects.filter(prototype__type='service',
                                                prototype__id=service.id)
        service.components = Component.objects.filter(prototype=service)
        service.config = PrototypeConfig.objects.filter(
            prototype=service, action=None).order_by('id')
    ...
    def get_object(self):
        ...
        for action in Action.objects.filter(prototype__id=obj_type.id):
            action.config = PrototypeConfig.objects.filter(
                prototype__id=obj_type.id, action=action)
            act_set.append(action)
        obj_type.actions = act_set
        obj_type.config = PrototypeConfig.objects.filter(prototype=obj_type,
                                                         action=None)
