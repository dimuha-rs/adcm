...
class ClusterHostList(PageView):
    ...
    ordering_fields = ('fqdn', 'state', 'provider__name',
...
class ClusterHostDetail(ListView):
    ...
    def check_host(self, cluster, host):
        if host.cluster != cluster:
            msg = "Host #{} doesn't belong to cluster #{}".format(
                host.id, cluster.id)
...
class ClusterBundle(ListView):
    ...
    def get(self, request, cluster_id):  # pylint: disable=arguments-differ
        ...
        shared = self.get_queryset().filter(shared=True).exclude(
            bundle=cluster.prototype.bundle)
        serializer_class = self.select_serializer(request)
        serializer = serializer_class(list(chain(bundle, shared)),
                                      many=True,
                                      context={
                                          'request': request,
                                          'cluster': cluster
...
class ClusterUpgrade(PageView):
    ...
    def get(self, request, cluster_id):  # pylint: disable=arguments-differ
        ...
        obj = cm.upgrade.get_upgrade(
            cluster, self.get_ordering(request, self.queryset, self))
        serializer = self.serializer_class(obj,
                                           many=True,
                                           context={
...
class ClusterServiceList(PageView):
    ...
    ordering_fields = ('state', 'prototype__display_name',
                       'prototype__version_order')
...
class HostComponentList(GenericAPIPermView, InterfaceView):
    ...
    def get(self, request, cluster_id):
        ...
        if self.for_ui(request):
            ui_hc = HostComponent()
            ui_hc.hc = hc
            serializer = serializer_class(ui_hc,
                                          context={
                                              'request': request,
                                              'cluster': cluster
                                          })
        else:
            serializer = serializer_class(hc,
                                          many=True,
                                          context={'request': request})
        return Response(serializer.data)
...
class ClusterConfigRestore(GenericAPIPermView):
    ...
    def patch(self, request, cluster_id, service_id, version):
        ...
        serializer = self.serializer_class(cl,
                                           data=request.data,
                                           context={'request': request})
        return update(serializer)
...
class ClusterConfigHistory(ListView):
    ...
    def get_obj(self, cluster_id, service_id):
        obj = get_obj_conf(cluster_id, service_id)
        return (obj, self.get_queryset().get(obj_ref=obj.config,
